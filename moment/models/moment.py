...
class MOMENT(nn.Module):
    def __init__(self, configs: Namespace | dict, **kwargs: dict):
        ...
        self.head = self._get_head("pre-training")
    ...
    def _get_huggingface_transformer(self, configs):
        class CustomT5EncoderModel(T5EncoderModel):
            def __init__(self, config: T5Config):
                ...
                self.encoder = T5Stack(encoder_config)  # Modified line: removed self.shared

        class CustomT5Model(T5Model):
            def __init__(self, config: T5Config):
                # Call the parent class's __init__ method.
                # This will initialize self.shared, self.encoder, and self.decoder
                # with self.shared being passed to both T5Stack instances.
                super().__init__(config)

                # Now, re-initialize self.encoder and self.decoder *without* passing self.shared.
                # This effectively overrides the initialization done by the parent's __init__.

                encoder_config = copy.deepcopy(config)
                encoder_config.is_decoder = False
                encoder_config.use_cache = False
                encoder_config.is_encoder_decoder = False
                # Modified line: removed self.shared
        ...
        if configs.transformer_backbone == "google/flan-t5-large":
            model_path = "./T5models/flan-t5-large/"
        if configs.transformer_backbone == "google/flan-t5-base":
            model_path = "./T5models/flan-t5-base/"
        if configs.transformer_backbone == "google/flan-t5-small":
            model_path = "./T5models/flan-t5-small/"
        if configs.transformer_backbone == "distill-t5-smallx1":
            model_path = "./T5models/distill-t5-smallx1/"
        if configs.transformer_backbone == "distill-t5-smallx2":
            model_path = "./T5models/distill-t5-smallx2/"
        if configs.getattr("randomly_initialize_backbone", False):
            model_config = T5Config.from_pretrained(model_path)
            # transformer_backbone = CustomT5Model(model_config)
            ...
            # transformer_backbone = CustomT5EncoderModel.from_pretrained(model_path)
            transformer_backbone = T5EncoderModel.from_pretrained(model_path)
        ...
        **kwargs,
        ...
        if self.configs.transformer_type == "encoder_decoder":
            outputs = self.encoder(
                ...
                output_hidden_states = debug
            )
        else:
            outputs = self.encoder(inputs_embeds=enc_in, attention_mask=attention_mask, output_hidden_states = debug)
        enc_out = outputs.last_hidden_state
        intermedia_out = outputs.hidden_states
    ...
    def forward(
        ...
        **kwargs,
        ...
        elif self.task_name == TASKS.DISTILLING:
            return self.pretraining(
                x_enc=x_enc, mask=mask, input_mask=input_mask, **kwargs
            )
        elif (
