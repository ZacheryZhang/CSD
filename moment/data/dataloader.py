...
def get_all_datasets(datasets_fraction: float = 1.0):
    ...
    for dataset in classification_datasets:
        if "Crop" in dataset or "Car" in dataset:
...
def get_timeseries_dataloader(args, **kwargs):
    ...
    if args.dataset_names == "all":
        assert (
            args.task_name == "pre-training" or args.task_name == "distilling"
        ...
        filtered_datasets = [ds for ds in dataset_classes if ds.length_timeseries >= args.seq_len]
        filtered__datasets = []
        for idx, ds in enumerate(filtered_datasets):
            try:
                length = len(ds)
                sample = ds[0]  # 尝试访问第一个样本
                filtered__datasets.append(filtered_datasets[idx])
            except Exception as e:
                print(f"Error in dataset {idx}: {e}")
        print(f"Filtered datasets count: {len(filtered__datasets)}")
        dataset = ConcatDataset(filtered__datasets)
        ...
        for kes in dataset_name_to_class.keys():
            if args.dataset_names in kes:
                dataset = dataset_name_to_class[kes](**vars(args))
