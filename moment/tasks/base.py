...
def print_model_details_with_shapes(model, dummy_input):
    # Dictionary to store input/output shapes
    shapes = {}

    # Register forward hooks to capture input/output shapes
    def register_hook(name):
        def hook(module, input, output):
            input_shape = (
                tuple(input[0].shape) if isinstance(input, tuple) and len(input) > 0
                else "N/A"
            )
            output_shape = (
                tuple(output.shape) if isinstance(output, torch.Tensor)
                else tuple(output[0].shape) if isinstance(output, tuple) and len(output) > 0
                else "N/A"
            )
            shapes[name] = {"input_shape": input_shape, "output_shape": output_shape}
    ...
    model.eval()
    model = model.to(model.configs.device)
    print(model.configs.device)
    input_mask = torch.ones_like(dummy_input).squeeze()
    dummy_input = dummy_input.to(model.configs.device)
    print(input_mask.shape)
    print(dummy_input.shape)
    with torch.no_grad():
        # try:
        _ = model(x_enc=dummy_input, input_mask=input_mask)
    ...
    # Print table with parameters, size, and shapes
    total_params = 0
    total_size_bytes = 0

    print(f"{'Layer':<40} {'Parameters':<15} {'Size (MB)':<10} {'Input Shape':<20} {'Output Shape':<20}")
    print("-" * 110)
    ...
    for name, module in model.named_modules():
        ...
        module_size_bytes = module_params * 4  # Assuming float32
        module_size_mb = module_size_bytes / (1024 ** 2)

        # Get shapes from the forward pass
        input_shape = shapes.get(name, {}).get("input_shape", "N/A")
        output_shape = shapes.get(name, {}).get("output_shape", "N/A")

        # Update totals
        if name in ["normalizer", "tokenizer", "patch_embedding", "encoder", "head"]:
            total_params += module_params
            total_size_bytes += module_size_bytes

        # Print details
        print(f"{name:<40} {module_params:<15} {module_size_mb:.4f} {str(input_shape):<20} {str(output_shape):<20}")

    # Print total
    total_size_mb = total_size_bytes / (1024 ** 2)
    print("-" * 110)
    print(f"{'Total':<40} {total_params:<15} {total_size_mb:.4f}")
class Tasks(nn.Module):
    def __init__(self, args, stu_args=None, cache_dir="/media3/cache", **kwargs):
        super(Tasks, self).__init__()
        self.args = args
        self.cache_dir = cache_dir
        os.makedirs(cache_dir, exist_ok=True)
        self.args.stu_task_name = self.args.stu_task_name if "stu_task_name" in self.args else None
        if self.args.stu_task_name == "distilling":
            self.stu_args = stu_args
    ...
    def _build_model(self):
        batch_size = 2048
        num_features = 1  # Assuming univariate time series as per RevIN num_features=1
        ...
        seq_len = self.model.seq_len  # From configs
        dummy_input = torch.randn(batch_size, num_features, seq_len).to(self.model.configs.device)
        print_model_details_with_shapes(self.model, dummy_input)

        if self.args.stu_task_name == "distilling":
            self.stu_model = MOMENT(configs=self.stu_args)
            seq_len = self.stu_model.seq_len  # From configs
            dummy_input = torch.randn(batch_size, num_features, seq_len).to(self.stu_model.configs.device)
            print_model_details_with_shapes(self.stu_model, dummy_input)
        if self.args.stu_task_name == "distilling":
            return self.model, self.stu_model
        else:
            return self.model
    ...
    def _get_dataloader(self, data_split: str = "train"):
        ...
        if self._dataloader.get(data_split) is not None:
            ...
            #     cache_path = os.path.join(self.cache_dir, f"{self.args.task_name}_{data_split}_dataloader.pkl")
            #
            #     if os.path.exists(cache_path):
            #         print(f"[CACHE] Loading {data_split} dataloader from disk cache.")
            #         with open(cache_path, "rb") as f:
            #             return pickle.load(f)

            print(f"[INFO] Building new {data_split} dataloader.")
            data_loader_args = deepcopy(self.args)
            data_loader_args.dataset_names = "/media3/Timeseries-PILE/classification/UCR/Crop/"
            ...
            #         pickle.dump(self._dataloader[data_split], f)
            return self._dataloader.get(data_split)
    ...
    def _distill_select_optimizer(self):
        if self.stu_args.optimizer_name == "AdamW":
            optimizer = optim.AdamW(
                self.stu_model.parameters(),
                lr=self.stu_args.init_lr,
                weight_decay=self.stu_args.weight_decay,
            )
        elif self.stu_args.optimizer_name == "Adam":
            optimizer = optim.Adam(
                self.stu_model.parameters(),
                lr=self.stu_args.init_lr,
                weight_decay=self.stu_args.weight_decay,
            )
        elif self.stu_args.optimizer_name == "SGD":
            optimizer = optim.SGD(
                self.stu_model.parameters(),
                lr=self.stu_args.init_lr,
                momentum=self.stu_args.momentum,
                weight_decay=self.stu_args.weight_decay,
            )
        else:
            raise NotImplementedError(
                f"Optimizer {self.stu_args.optimizer_name} not implemented"
            )
        return optimizer
    ...
    def load_pretrained_moment(
        ...
        return True
    def load_released_moment(
        self, do_not_copy_head: bool = True
    ):
        pretraining_args = deepcopy(self.args)

        checkpoint = BaseModel.load_released_weights(
            rpath=pretraining_args.pretraining_run_name
        )
        pretrained_model = MOMENT(configs=pretraining_args)
        ...
        for (name_p, param_p), (name_f, param_f) in zip(
            ...
            if (name_p == name_f) and (param_p.shape == param_f.shape):
                if do_not_copy_head and name_p.startswith("head"):
                    ...
                    param_f.data = param_p.data
        self.step_freeze_model_parameters(self.args.levels)
        # self.step_freeze_model_parameters_r(self.args.levels)
        # self.freeze_model_parameters()  # Freeze model parameters based on fine-tuning mode

        return True
    def step_load_released_moment(
        self, do_not_copy_head: bool = True
    ):
        pretraining_args = deepcopy(self.args)
        ...
        self.step_freeze_model_parameters(self.args.levels)  # Freeze model parameters based on fine-tuning mode

        return True

    def step_freeze_model_parameters(self, step):
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                for level in self.args.levels:
                    if f"block.{level}" in name:
        ...
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                ...
                print("Frozen:", name)
        print("=====================================")
    def step_freeze_model_parameters_r(self, step):
        totals = list(range(0,24))
        for level in self.args.levels:
            totals.remove(level)
        print("totals", totals)
        with torch.no_grad():
    ...
    def freeze_model_parameters(self):
        if self.args.finetuning_mode == "linear-probing":
            for name, param in self.model.named_parameters():
                if not name.startswith("head"):
                    param.requires_grad = False
        elif self.args.finetuning_mode == "distill":
            for name, param in self.model.named_parameters():
                param.requires_grad = False
