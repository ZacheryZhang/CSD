...
class ImputationFinetuning(Tasks):
    ...
    def train(self):
        ...
        with open(f"{self.args.levels}_zero_shot_{eval_metrics.test_loss}_{eval_metrics.val_loss}_{eval_metrics.train_loss}.txt",'w') as f:
            pass

        print("before ft", self.args.levels)
        print(eval_metrics)
        opt_steps = 0
        cur_epoch = 0
    ...
    def evaluate_and_log(self):
        eval_metrics = self.evaluate_model()
        print("after ft", self.args.levels)
        print(eval_metrics)
        with open(f"{self.args.levels}_fine_tune_{eval_metrics.test_loss}_{eval_metrics.val_loss}_{eval_metrics.train_loss}.txt",'w') as f:
            pass
        self.logger.log(
            {
                "train_loss": eval_metrics.train_loss,
